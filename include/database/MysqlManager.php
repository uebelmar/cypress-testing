...
class MysqlManager extends DBManager
{
	...
	{
		LoggerManager::getLogger()->debug('Fetching table list');
	...
	*/
	...
	{
		LoggerManager::getLogger()->debug("creating temp table for [$table]...");
		...
		LoggerManager::getLogger()->debug('inserting temp dataset...');
	...
	{
		LoggerManager::getLogger()->debug("verifying ALTER TABLE");
		...
		if(strpos(strtoupper($query), 'DROP PRIMARY KEY') !== false) {
			LoggerManager::getLogger()->debug("Skipping DROP PRIMARY KEY");
			return '';
		}
		...
		if (strpos($tempTableTestQuery, 'idx') === false) {
			...
			LoggerManager::getLogger()->debug('testing query on temp table: ['.$tempTableTestQuery.']');
			...
			LoggerManager::getLogger()->debug('testing query on temp table: ['.$tempTableTestQuery_idx.']');
	...
	{
		LoggerManager::getLogger()->debug("verifying $querytype statement");
