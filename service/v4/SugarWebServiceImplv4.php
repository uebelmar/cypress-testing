...
class SugarWebServiceImplv4 extends SugarWebServiceImplv3_1 {
    ...
    public function login($user_auth, $application = '', $name_value_list = array()){
        LoggerManager::getLogger()->info("Begin: SugarWebServiceImpl->login({$user_auth['user_name']}, $application, ". print_r($name_value_list, true) .")");
        ...
        if($success)
        {
            ...
            LoggerManager::getLogger()->info('End: SugarWebServiceImpl->login - successful login');
        ...
        self::$helperObject->setFaultObject($error);
        LoggerManager::getLogger()->error('End: SugarWebServiceImpl->login - failed login');
    }
     ...
     * @exception 'SoapFault' -- The SOAP error, if any
     */
    function get_entry_list($session, $module_name, $query, $order_by,$offset = 0, $select_fields = array(), $link_name_to_fields_array = array(), $max_results = 0, $deleted = false, $favorites = false ){
