...
VALUES('{$call->id}','{$call->channel}','{$call->direction}','{$call->state}','{$call->callednumber}','{$call->callernumber}')");
     ...
     * @return mixed
     */
    public function notifySocket($room, $data, $path = '') {
        

        $url = SpiceConfig::getInstance()->config['core']['socket_backend'] . $path;
        $body = ['room' => $room, 'message' => $data, 'sysid' => SpiceConfig::getInstance()->config['core']['socket_id']];

        if ($room == 'alcatel') {
            LoggerManager::getLogger()->fatal('Alcatel Restart Request to: ' . $url);
        }
