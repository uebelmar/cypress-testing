...
class SystemDeploymentCR extends SugarBean
{
    ...
    function getList($data, $listType = null)
    {
        ...
        switch($listType){
            ...
            case 'active': //active means status is not completed AND some log entries are found
                $queryCount = "SELECT COUNT(crs.id) cnt FROM systemdeploymentcrs crs 
INNER JOIN systemdeploymentcrdbentrys entries ON entries.systemdeploymentcr_id = crs.id 
WHERE crs.deleted=0 AND entries.deleted = 0";
                $query = "SELECT crs.* FROM systemdeploymentcrs crs 
        INNER JOIN systemdeploymentcrdbentrys entries ON entries.systemdeploymentcr_id = crs.id AND entries.deleted=0
WHERE crs.deleted = 0 AND crs.crstatus <> '3' GROUP BY crs.id ORDER BY entries.date_modified DESC"; //3 = completed
                break;

            default:
                $queryCount= "SELECT COUNT(id) cnt FROM systemdeploymentcrs WHERE deleted = 0";
                $query = "SELECT * FROM systemdeploymentcrs WHERE deleted = 0 ORDER BY date_entered DESC";
        }

        $total = $db->fetchByAssoc($db->query($queryCount));
        $res = $db->limitQuery($query, $start, 50);
        $auth_rights = 0;
...
// table krp_kcr no longer exists)
...
//            while ($row2 = $db->fetchByAssoc($res1)) $auth_rights = 2; //view
    ...
    function getDBEntriesSQL()
    {
        ...
        // CR1000420 get version from related release
        $release = BeanFactory::getBean('SystemDeploymentReleases', $this->systemdeploymentrelease_id);
                    ...
                    if($entrydata) {
                        ...
                        foreach ($entrydata as $field => $value) {
                            ...
                            if ($field == 'version' && $value == '') {
                                // CR1000420 set verion from release name
                                if($release && $release->name){
                                    $value = $release->name;
                                }
                                // force number
                                if(isset(SpiceConfig::getInstance()->config['force_reference_version']) && !empty(SpiceConfig::getInstance()->config['force_reference_version'])){
                                    $value = SpiceConfig::getInstance()->config['force_reference_version'];
                                }
                            }
