...
$current_user = AuthenticationController::getInstance()->getCurrentUser();
	...
	if($user->authenticate_user($password)){
		...
		LoggerManager::getLogger()->fatal("SECURITY: failed attempted login for $user_name using SOAP api");
...
function track_email($user_name, $password,$parent_id, $contact_ids, $date_sent, $email_subject, $email_body)
{
	...
	LoggerManager::getLogger()->info("In track email: username: $user_name contacts: $contact_ids date_sent: $date_sent");
...
//
//
//	$seed_user = \SpiceCRM\data\BeanFactory::getBean('Users')
//	$user_id = $seed_user->retrieve_user_id($user_name);
//	$case = new aCase();
