...
namespace SpiceCRM\modules\SystemDeploymentSystems;
...
class SystemDeploymentSystem extends SugarBean
{

    var $new_schema = true;
    var $module_dir = 'SystemDeploymentSystems';
    var $object_name = 'SystemDeploymentSystem';
    var $table_name = 'systemdeploymentsystems';
    ...
    function add_system_link($id, $link)
    {
        ...
        $guid = create_guid();
        $sql = "INSERT INTO systemdeploymentsystems_systemdeploymentsystems (id, systemdeploymentsystem1_id, systemdeploymentsystem2_id, date_modified) VALUES ('$guid', '$id', '$link', NOW())";
        $db->query($sql);
        return $guid;
    }
    ...
    function get_dist_data($id)
    {
        ...
        $res = $db->query("SELECT * FROM systemdeploymentsystems WHERE deleted = 0");
        ...
        $res = $db->query("SELECT * FROM systemdeploymentsystems_systemdeploymentsystems WHERE deleted = 0");
        while ($row = $db->fetchByAssoc($res)) {
            $return['links'][] = $row;
        }
    ...
    function import_systems($params)
    {
        ...
        //\SpiceCRM\includes\Logger\LoggerManager::getLogger()->fatal("KREST import_systems:\n".print_r($data,true));
        if (count($data['systems']) > 0) {
            $db->query("DELETE FROM systemdeploymentsystems");
            foreach ($data['systems'] as $sys) {
                $sql = "INSERT INTO systemdeploymentsystems (id,name,date_entered,date_modified,modified_user_id,created_by,description,assigned_user_id,sys_username,sys_password,master_flag,url,type,this_system) VALUES ('$sys->id','$sys->name','$sys->date_entered','$sys->date_modified','$sys->modified_user_id','$sys->created_by','$sys->description','$sys->assigned_user_id','$sys->sys_username','$sys->sys_password',$sys->master_flag,'$sys->url','$sys->type',$sys->this_system)";
                $db->query($sql);
            }
        }
        if (count($data['links']) > 0) {
            $db->query("DELETE FROM systemdeploymentsystems_systemdeploymentsystems");
            foreach ($data['links'] as $link) {
                $sql = "INSERT INTO systemdeploymentsystems_systemdeploymentsystems (id,systemdeploymentsystem1_id,systemdeploymentsystem2_id,date_modified) VALUES ('$link->id','$link->systemdeploymentsystem1_id','$link->systemdeploymentsystem2_id','$link->date_modified')";
                $db->query($sql);
            }
        }
        return "OK";
    ...
    function get_systems()
    {
        ...
        if (!$this_found) {
            ...
            $this->url = SpiceConfig::getInstance()->config['site_url'];
            $this->this_system = 1;
            $this->master_flag = 1;
            $id = $this->save();
            $systems[] = array(
    ...
    function get_source_systems()
    {
        $db = DBManagerFactory::getInstance();
        $systems = array();
        $row = $db->fetchByAssoc($db->query("SELECT id FROM systemdeploymentsystems WHERE deleted = 0 AND this_system = 1"));
        if (!empty($row['id'])) {
            $this->retrieve($row['id']);
            $bean_list = $this->get_linked_beans('systemdeploymentsystems_link', 'SystemDeploymentSystem');
    ...
    function del_system($id)
    {
        ...
        $db->query("UPDATE systemdeploymentsystems_systemdeploymentsystems SET deleted = 1 WHERE systemdeploymentsystem2_id = '$id'");
    }
    ...
    function get_release_packages($params)
    {
        ...
        $rp = BeanFactory::getBean('SystemDeploymewntPackages');
        if ($params['remote'] === '1') {
            $bean_list = $rp->get_list('release_date', "systemdeploymentpackages.rpstatus = '7'",$start,50);
        } else {
            $bean_list = $rp->get_list('release_date','',$start,50);
        }
        $packages = array();
        $fetched = array();
        $res = $db->query("SELECT systemdeploymentpackage_id FROM kdeploypack_sysstatus WHERE systemdeploymentsystem_id = '" . $params['sysid'] . "'");
        while ($row = $db->fetchByAssoc($res)) {
            $fetched[$row['systemdeploymentpackage_id']] = $row['systemdeploymentpackage_id'];
        }
        foreach ($bean_list['list'] as $bean) {
            if (empty($fetched[$bean->id])) {
                $items = array();
                $res = $db->query("SELECT id,ckey,ctype, date_modified FROM kdeploypack_items WHERE systemdeploymentpackage_id = '$bean->id' AND deleted = 0");
                ...
                $source = $db->fetchByAssoc($db->query("SELECT name FROM systemdeploymentsystems WHERE id = '" . $bean->source_system . "'"));
    ...
    function get_remote_packages($system_id)
    {
        ...
        if (!empty($system_id)) {
            ...
            $sysid = $db->fetchByAssoc($db->query("SELECT id FROM systemdeploymentsystems WHERE this_system = 1 AND deleted = 0"));
            $options = array(
                CURLOPT_RETURNTRANSFER => true,         // return web page
                CURLOPT_HEADER => false,        // don't return headers
                CURLOPT_POST => false
    ...
    function fetch_remote_package($id, $system, $data)
    {
        ...
        foreach ($data['items'] as $item) {
            $db->query("INSERT INTO kdeploypack_items (id,ckey,ctype,systemdeploymentpackage_id,date_modified) VALUE ('" . $item['id'] . "','" . $item['ckey'] . "','" . $item['ctype'] . "','$id','" . $item['date_modified'] . "')");
        }
        $sysid = $db->fetchByAssoc($db->query("SELECT id FROM systemdeploymentsystems WHERE this_system = 1 AND deleted = 0"));
    ...
    function notify_source_systems($package_id, $status)
    {
        ...
        if (!empty($row['id'])) {
            ...
            $bean_list = $this->get_linked_beans('systemdeploymentsystems_link', 'SystemDeploymentSystem');
            $row2 = $db->fetchByAssoc($db->query("SELECT id FROM systemdeploymentsystems WHERE deleted = 0 AND master_flag = 1"));
            require_once('data/BeanFactory.php');
            $master = BeanFactory::getBean('SystemDeploymentSystems', $row2['id']);
            if (!empty($master->id)) {
                $bean_list[] = $master;
            }
    ...
    function update_remote_package_status($pack_id, $status, $sysid)
    {
        ...
        $res = $db->query("SELECT * FROM kdeploypack_sysstatus WHERE systemdeploymentpackage_id = '$pack_id' AND systemdeploymentsystem_id = '$sysid'");
        $entrys = array();
        ...
        if (count($entrys) > 0) {
            ...
            $db->query("INSERT INTO kdeploypack_sysstatus (id, systemdeploymentpackage_id, systemdeploymentsystem_id, pstatus,date_modified,history) VALUES ('" . create_guid() . "','$pack_id','$sysid','$status','$date',0)");
        } else {
            $db->query("INSERT INTO kdeploypack_sysstatus (id, systemdeploymentpackage_id, systemdeploymentsystem_id, pstatus,date_modified,history) VALUES ('" . create_guid() . "','$pack_id','$sysid','$status','$date',0)");
        }
        return $return;
    ...
    {
        $db = DBManagerFactory::getInstance();
        $sql = "SELECT * FROM kdeploypack_contents WHERE kdeploypack_item_id IN (SELECT id FROM kdeploypack_items WHERE systemdeploymentpackage_id = '$id')";
        $res = $db->query($sql);
        $content = array();
        while ($row = $db->fetchByAssoc($res)) {
            $content[] = $row;
    ...
    function get_items_from_packages($packages)
    {
        $db = DBManagerFactory::getInstance();
        $items = array();
        $res = $db->query("SELECT * FROM kdeploypack_items WHERE systemdeploymentpackage_id IN ('" . implode("','", $packages) . "') AND deleted = 0 ORDER BY date_modified DESC");
        while ($row = $db->fetchByAssoc($res)) {
    ...
    function write_files($packs, $remote)
    {
        ...
        $return = array('status' => 'ok', 'msg' => '');
        if (!$remote) {
            $res = $db->fetchByAssoc($db->query("SELECT add_systems FROM systemdeploymentsystems WHERE this_system = 1"));
    ...
    function repair($packs, $remote)
    {
        ...
        $repair_array = array();
        foreach ($packs as $pack_id) {
            $row = $db->fetchByAssoc($db->query("SELECT repairs, repair_modules FROM systemdeploymentpackages WHERE id = '$pack_id'"));
    ...
    function release_package_history($pack_id)
    {
        ...
        include('custom/application/Ext/Language/en_us.lang.ext.php');
        $list = array();
        $res = $db->query("SELECT stat.id id, stat.systemdeploymentsystem_id system_id, sys.name system, stat.pstatus status_raw, stat.date_modified, stat.history FROM kdeploypack_sysstatus stat INNER JOIN systemdeploymentsystems sys ON stat.systemdeploymentsystem_id = sys.id WHERE stat.systemdeploymentpackage_id = '$pack_id'");
        while ($row = $db->fetchByAssoc($res)) {
            $row['status'] = $app_list_strings['rpstatus_dom'][$row['status_raw']];
            /*$row['text'] = $app_list_strings['rpstatus_dom'][$row['status_raw']];
            $res2 = $db->query("SELECT stat.id id, stat.systemdeploymentsystem_id system_id, sys.name system, stat.pstatus status_raw, stat.date_modified, stat.history FROM kdeploypack_sysstatus stat INNER JOIN systemdeploymentsystems sys ON stat.systemdeploymentsystem_id = sys.id WHERE stat.systemdeploymentpackage_id = '$pack_id' AND stat.history = 1 AND stat.systemdeploymentsystem_id = '".$row['system_id']."'");
    ...
    function getUpdates(){
        ...
        $installedSW = array();
        $res = $db->query("SELECT * FROM systemdeploymentpackages WHERE deleted = 0 AND software_name IS NOT NULL");
    ...
    function getLatestSwVersions($sw){
        ...
        $res = $db->query("SELECT * FROM systemdeploymentsystems WHERE type = 'repo' AND deleted = 0");
        while($row = $db->fetchByAssoc($res)){
            $options = array(
                CURLOPT_RETURNTRANSFER => true,         // return web page
                CURLOPT_HEADER => false,        // don't return headers
    ...
    function latestSwVersions($data){
        ...
        foreach($data as $swHash){
            $latestSW = array();
            $res = $db->query("SELECT * FROM systemdeploymentpackages WHERE deleted = 0 AND software_hash = '$swHash' AND rpstatus = '7'");
            $latestVersions = array();
            while($row = $db->fetchByAssoc($res)){
    ...
    function recieveZipFromForm($data){
        ...
        if(isset($_FILES['uploadzippackage']) && $_FILES['uploadzippackage']['error'] == UPLOAD_ERR_OK){
            ...
            if(!file_exists(SpiceConfig::getInstance()->config['upload_dir'].'temp')){
                mkdir(SpiceConfig::getInstance()->config['upload_dir'].'temp');
            }
            if(move_uploaded_file($tmp_name, SpiceConfig::getInstance()->config['upload_dir'].'temp'.$name)){
                $return = $this->processZipToRP(SpiceConfig::getInstance()->config['upload_dir'].'temp'.$name);
            }else{
                $return = array('success' => false, 'error' => 'Error: not able to move file to '. SpiceConfig::getInstance()->config['upload_dir'].'temp'.$name);
            }
        }else{
    ...
    function processZipToRP($dir){
        ...
        $workdir = SpiceConfig::getInstance()->config['upload_dir'].'temp';
        unzip($dir,$workdir);
