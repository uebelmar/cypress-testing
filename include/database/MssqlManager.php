...
class MssqlManager extends DBManager
{
    ...
    private function getTableNameFromModuleName($module_str, $sql)
    {
        ...
        LoggerManager::getLogger()->debug("Module being processed is " . $module_str);
        ...
        if (($module_str != 'Reports' && $module_str != 'SavedReport') && isset($beanList[$module_str])  &&  isset($beanFiles[$beanList[$module_str]])){
            ...
            if(empty($tbl_name)){
                LoggerManager::getLogger()->debug("Could not find table name for module $module_str. ");
                $tbl_name = $module_str;
            }
        }
        else {
            ...
            LoggerManager::getLogger()->debug("Could not find table name from module in request, retrieve from passed in sql");
            $tbl_name = $module_str;
            $sql = strtolower($sql);
            ...
            if ($fromLoc>0){
                ...
                if ($next_space > 0) {
                    ...
                    if(empty($tbl_name)){
                        LoggerManager::getLogger()->debug("Could not find table name sql either, return $module_str. ");
                        $tbl_name = $module_str;
                    }
                }
        ...
        //return table name
        LoggerManager::getLogger()->debug("Table name for module $module_str is: ".$tbl_name);
        return $tbl_name;
    }
