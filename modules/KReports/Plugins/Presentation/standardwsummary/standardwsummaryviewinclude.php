...
class kreportpresentationstandardws extends kreportpresentationplugin {
    ...
    public function display(&$thisReport) {
        ...
        $viewJS .= '<script type="text/javascript" src="custom/modules/KReports/Plugins/Presentation/standardwsummary/js/viewstandardwsummary' . (SpiceConfig::getInstance()->config['KReports']['debug'] ? '_debug' : '') . '.js"></script>';
    ...
    public function getExportData($thisReport, $dynamicols = '', $renderFields = true, $parentBean = null, $pluginName = null) {
        ...
        if ($thisReport->kQueryArray->summarySelectString != '') {
            $totalArray = DBManagerFactory::getInstance()->fetchByAssoc(DBManagerFactory::getInstance()->query($thisReport->kQueryArray->summarySelectString));

            foreach ($fieldArray as $fieldId => $fieldData) {
                if ($fieldData['renderer'] != ''){
                    //PHP7 - 5.6 COMPAT
                    //ORIGINAL: $exportData['datasetsummaries']['main'][$fieldId] = $thisReportRenderer->$fieldData['renderer']($fieldId, $totalArray);
                    $rendererFn = $fieldData['renderer'];
                    $exportData['datasetsummaries']['main'][$fieldId] = $thisReportRenderer->$rendererFn($fieldId, $totalArray);
                    //END
                }
                else
