...
class AuthenticationKRESTController
{
    ...
    public function AuthChangePassword($req, $res, $args)
    {
        ...
        if (AuthenticationController::getInstance()->canChangePassword === false) {
            throw new UnauthorizedException("Password Change unavailable");
        }
        $userAuthenticationController = new UserAuthenticate();
        $userAuthenticationController->changePassword($parsedBody['username'], $parsedBody['password'], $parsedBody['newPassword']);
    ...
    public function AuthSetNewPassword($req, $res, $args)
    {
        ...
        $parsedBody = $req->getParsedBody();
        /** @var User $userObj */
        $userObj = \SpiceCRM\data\BeanFactory::getBean("Users", $args['id']);
        if (!$userObj) {
            throw new \SpiceCRM\includes\ErrorHandlers\UnauthorizedException("No User with id " . $args['id']);
        }
