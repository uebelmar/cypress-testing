...
class SpiceUIConfServerController
{
    ...
    static function getLanguageLabels($req, $res, $args){
        ...
        //$records = $db->query("SELECT * FROM $table WHERE version='$version'");
        $records = $db->query("SELECT ll.id, ll.name, ll.package, ll.version, lt.syslanguage, lt.translation_default, lt.translation_short, lt.translation_long FROM syslanguagelabels ll, syslanguagetranslations lt WHERE ll.id = lt.syslanguagelabel_id AND lt.syslanguage in ('".implode("','", $langs)."')");
