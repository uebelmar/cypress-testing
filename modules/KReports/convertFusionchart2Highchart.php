...
class KReporterFusionChartSupport {
    ...
    public $mapFusion2HighchartType = array(
        ...
        'MSCombiDY2D' => 'column',
        'Marimekko' => 'serial_column_marimekko', //AmChart!
        'Bar2D' => 'bar',
        'Line' => 'line',
        'Pie2D' => 'pie',
        'Pie3D' => 'pie',
...
//echo '<pre>---------------------';
        foreach($visObj as $layout => $charts){
            $chartLibName = 'highcharts';

            if($charts->fusioncharts->type == 'Marimekko')
                $chartLibName = 'amcharts';

            if($visObj->plugin == 'fusioncharts') {
                $visObj->plugin = $chartLibName;
            }
...
// echo '<pre>F '.print_r($charts->fusioncharts, true);
                ...
                $visObj->$layout->$chartLibName = $charts->fusioncharts;

// echo '<pre>fill H '.print_r($visObj->$layout->highcharts, true);

                $visObj->$layout->$chartLibName->type = $this->mapChartTypeFusion2Highchart($charts->fusioncharts->type);
                if($visObj->$layout->$chartLibName->type == "" && !empty($charts->fusioncharts->type)){
                    echo "<br />Could not map chart type ".json_encode($charts->fusioncharts). " in report with ID ".$kreport_id;
                }
                if(isset($visObj->$layout->plugin) && $visObj->$layout->plugin == 'fusioncharts') $visObj->$layout->plugin = $chartLibName;

                //clean up dataseries config
                if(isset($visObj->$layout->$chartLibName->dataseries)){
                    foreach($visObj->$layout->$chartLibName->dataseries as $idx => $dataserie){
                ...
                if(preg_match("/3D$/", $originalFusionChartType)){
                    $visObj->$layout->$chartLibName->options->{'3d'} = "on";
                }

                //convert legend param from charttype
                if($visObj->$layout->$chartLibName->options->legend == "on"){
                    $visObj->$layout->$chartLibName->legend = "bottom";
                    unset($visObj->$layout->$chartLibName->options->legend);
                }

                //force colorBypoint
                $visObj->$layout->$chartLibName->options->colorByPoint == "on";
