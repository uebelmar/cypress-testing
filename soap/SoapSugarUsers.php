...
function login($user_auth, $application)
{
    ...
    if ($isLoginSuccess) {
        ...
        if (!empty($user) && !empty($user->id) && !$user->is_group) {
            $success = true;
            $current_user = AuthenticationController::getInstance()->setCurrentUser($user);
        } // if
    ...
    LoggerManager::getLogger()->fatal('SECURITY: User authentication for ' . $user_auth['user_name'] . ' failed');
    LogicHook::initialize();
    $GLOBALS['logic_hook']->call_custom_logic('Users', 'login_failed');
    return array('id' => -1, 'error' => $error);
