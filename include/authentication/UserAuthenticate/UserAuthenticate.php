...
class UserAuthenticate
{
    ...
    {
        //todo $new_password should be checked for password complexity

        try {
            $userObj = $this->authenticate($username, $currentPwd);
            $userObj->retrieve($userObj->id);
            if ($userObj->external_auth_only === "1") {
                throw new ForbiddenException("Password Change not allowed");
            }
            $this->setNewPassword($userObj, $newPwd, $sendByEmail, false);
        } catch (UnauthorizedException $e) {
            //todo clarify how to send array
            throw new ForbiddenException("MSG_CURRENT_PWD_NOT_OK, current password not OK");
        }
        return true;
    }
