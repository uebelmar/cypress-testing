...
class PostgreSQLManager extends DBManager
{
        ...
        switch ($type){
            ...
            case 'clustered':
                if ($drop)
                    ...
                    $sql = "CREATE INDEX IF NOT EXISTS {$name} ON {$table} ({$fields});";
                break;
            // constraints as indices
            case 'unique':
                if ($drop)
                    $sql = "ALTER TABLE {$table} DROP CONSTRAINT IF EXISTS {$name} CASCADE;";
                else
    ...
    public function compareVarDefs($fielddef1, $fielddef2, $ignoreName = false)
    {
        foreach ( $fielddef1 as $key => $value ) {
            ...
            {
                //Catch type_map exceptions:
                //Db might return character and not char
                if($key == 'type'){
                    if($fielddef2[$key] == 'decimal') $fielddef2[$key] = 'numeric';
                    if($fielddef2[$key] == 'double') $fielddef2[$key] = 'double precision';
                    if($fielddef2[$key] == 'char') $fielddef2[$key] = 'character';
                    if($fielddef2[$key] == 'varchar') $fielddef2[$key] = 'character varying';
                    if($fielddef2[$key] == 'datetime' || $fielddef2[$key] == 'datetimecombo' || $fielddef2[$key] == 'timestamp') $fielddef2[$key] = 'timestamp without time zone';
                }
                // if the length in db is not set, then it's probably a numeric type. check and ignore it
                if($key == 'len'){
                    if(($fielddef2['type'] == 'timestamp' || $fielddef2['type'] == 'double precision' || $fielddef2['type'] == 'integer' || $fielddef2['type'] == 'smallint' || $fielddef2['type'] == 'bigint') && $fielddef1['type']==$fielddef2['type'])
                        continue;
                }
