...
class SystemDeploymentPackage extends SugarBean {

    var $new_schema = true;
    var $module_dir = 'SystemDeploymentPackages';
    var $object_name = 'SystemDeploymentPackage';
    var $table_name = 'systemdeploymentpackages';
    ...
    function getPrintOut() {
        ...
        $ss->assign('SystemDeploymentPackage', $this);
        ...
        return $ss->fetch('modules/SystemDeploymentPackages/tpls/releasedocument.tpl');
    }
    ...
    function create_package() {
        ...
        if (is_dir(SpiceConfig::getInstance()->config['upload_dir'] . $this->id))
            $this->removedir(SpiceConfig::getInstance()->config['upload_dir'] . $this->id);

        $linkedPackages = $this->get_linked_beans('kchangerequests', 'KDeploymentCR');

        mkdir(SpiceConfig::getInstance()->config['upload_dir'] . $this->id);
        mkdir(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/files');
        $fileArray = array();
        $dbDefinitions = array();
        $dbentriesArray = array();
        foreach ($linkedPackages as $thisLinkedPackage) {
            foreach ($thisLinkedPackage->fileArray as $thisFile) {
                ...
                //$this->smartCopy(str_replace('*', '', $thisFile['name']), \SpiceCRM\includes\SugarObjects\SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/files/' . $thisGuid);
                $this->smartCopy(str_replace('*', '', $thisFile['name']), SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/files/' . substr(str_replace('*', '', $thisFile['name']), 2));
                $fileArray[] = array(
                    'type' => $thisFile['type'],
        ...
        $fp = fopen(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/manifest.php', 'w+');
        fwrite($fp, $this->write_patch_manifest($fileArray));
        fclose($fp);

        // add the releasepackager version
        if (!empty($this->release_version)) {
            mkdir(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/files/custom/Extension/application/Ext/Include', '0755', true);
            $fp = fopen(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/files/custom/Extension/application/Ext/Include/KReleaseManagerVersion.php', 'w+');
            fwrite($fp, $this->write_versionfile($fileArray));
            fclose($fp);
        }
        // handle SQLs
        if (count($dbentriesArray) > 0) {
            mkdir(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/scripts');

            write_array_to_file('dbrecords', $dbentriesArray, SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/scripts/dbrecords.php');
            write_array_to_file('dbdefinitions', $dbDefinitions, SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/scripts/dbdefinitions.php');

            $this->smartCopy('modules/SystemDeploymentPackages/templates/post_install.php', SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/scripts/post_install.php');
        }

        // create the archive
        require_once('include/utils/php_zip_utils.php');
        zip_dir(SpiceConfig::getInstance()->config['upload_dir'] . $this->id, SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/' . $this->id . '.zip');


        $filename = str_replace(' ', '_', $this->name) . '_' . date('Ymd_His') . '.zip';
        header('Content-type: application/zip');
        header("Content-Transfer-Encoding: binary");
        header("Content-Length: " . filesize(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/' . $this->id . '.zip'));
        header('Content-Disposition: attachment; filename=' . $filename);

        readfile(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/' . $this->id . '.zip');
    }
    ...
    {
        global $beanFiles, $beanList, $dictionary;
    ...
    function write_patch_manifest($copyArray)
    {
        ...
        return $template->fetch('modules/SystemDeploymentPackages/templates/patch_manifest.php');
    }
    ...
    function write_versionfile()
    {
        $template = new Sugar_Smarty();
        $template->assign('version', $this->release_version);
        return $template->fetch('modules/SystemDeploymentPackages/templates/KReleaseManagerVersion.php');
    }

    function getFileChanges($source = '') {
        
        if (empty($source)) {
            $source = SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/files';
            unlink(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/package_dif_'.$this->id.'.html');
            file_put_contents(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/package_dif_'.$this->id.'.html', "<link href='http://fonts.googleapis.com/css?family=Open+Sans' rel='stylesheet' type='text/css'><style>body {font-family: 'Open Sans', sans-serif; padding: 10px;} </style>" . PHP_EOL, FILE_APPEND);
            file_put_contents(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/package_dif_'.$this->id.'.html', "<h1>Changelog Package ".$this->name."</h1>" . PHP_EOL, FILE_APPEND);
        }
        if (is_file($source)) {
    ...
    function trackFileChanges($srcFile) {
        ...
        if (!empty(SpiceConfig::getInstance()->config['KReleaseManager']['reference'])) {

            file_put_contents(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/package_dif_'.$this->id.'.html', '<h2>File: ' . str_replace(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/files/', '', $srcFile) . '</h2>' . PHP_EOL . PHP_EOL, FILE_APPEND);

            $refFile = str_replace(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/files', SpiceConfig::getInstance()->config['KReleaseManager']['reference'], $srcFile);

            if (file_exists($refFile)) {
                xdiff_file_diff($refFile, $srcFile, SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/tmp.dif', 3, true);
                /*
                  ob_start();
                  readfile(\SpiceCRM\includes\SugarObjects\SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/tmp.dif');
                  $change =  ob_get_contents();
                  ob_end_clean();
                 */
                if (filesize(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/tmp.dif') == 0)
                    file_put_contents(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/package_dif_'.$this->id.'.html', 'files are identical<br>' . PHP_EOL, FILE_APPEND);
                else {
                    $file = fopen(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/tmp.dif', "r");
                    while (!feof($file)) {
                        $line = fgets($file);
                        ...
                        $line .= '<br>';
                        file_put_contents(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/package_dif_'.$this->id.'.html', $line . PHP_EOL, FILE_APPEND);
                    }
                    fclose($file);
                }
            } else {
                file_put_contents(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/package_dif_'.$this->id.'.html', 'file added<br>' . PHP_EOL, FILE_APPEND);
            }

            file_put_contents(SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/package_dif_'.$this->id.'.html', PHP_EOL . '-----' . PHP_EOL, FILE_APPEND);
        }
    }
    ...
    public function retrieve($id = -1, $encode = true, $deleted = true, $relationships = true) {
        $thisResult = parent::retrieve($id, $encode, $deleted, $relationships);
    ...
    function release_package($id)
    {
        ...
        foreach ($linkedPackages as $thisLinkedPackage) {
            ...
            while ($thisFile = $db->fetchByAssoc($fileres)) {
                $this->smartCopyDb(str_replace('*', '', $thisFile['name']), SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/files/' . substr(str_replace('*', '', $thisFile['name']), 2));
            }
            ...
            while ($thisDBEntry = $db->fetchByAssoc($dbres)) {
                ...
                if ($dbRecord) {
                    ...
                    $db->query("INSERT INTO kdeploypack_items (id,systemdeploymentpackage_id,ckey,ctype,date_modified) VALUES ('$item_id','$this->id','" . $thisDBEntry['tablename'] . "/" . $thisDBEntry['tablekey'] . "','db', sysdate())");
        ...
        /*
        if (count($dbentriesArray) > 0) {
            mkdir(\SpiceCRM\includes\SugarObjects\SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/scripts');

            write_array_to_file('dbrecords', $dbentriesArray, \SpiceCRM\includes\SugarObjects\SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/scripts/dbrecords.php');
            write_array_to_file('dbdefinitions', $dbDefinitions, \SpiceCRM\includes\SugarObjects\SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/scripts/dbdefinitions.php');

            $this->smartCopy('modules/SystemDeploymentPackages/templates/post_install.php', \SpiceCRM\includes\SugarObjects\SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/scripts/post_install.php');
        }

        // create the archive
        require_once('include/utils/php_zip_utils.php');
        zip_dir(\SpiceCRM\includes\SugarObjects\SpiceConfig::getInstance()->config['upload_dir'] . $this->id, \SpiceCRM\includes\SugarObjects\SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/' . $this->id . '.zip');


        $filename = str_replace(' ', '_', $this->name) . '_' . date('Ymd_His') . '.zip';
        header('Content-type: application/zip');
        header("Content-Transfer-Encoding: binary");
        header("Content-Length: " . filesize(\SpiceCRM\includes\SugarObjects\SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/' . $this->id . '.zip'));
        header('Content-Disposition: attachment; filename=' . $filename);

        readfile(\SpiceCRM\includes\SugarObjects\SpiceConfig::getInstance()->config['upload_dir'] . $this->id . '/' . $this->id . '.zip');
        */
    }
    ...
    function smartCopyDb($source, $dest, $options = array('folderPermission' => 0755, 'filePermission' => 0755))
    {
        ...
        if (is_file($source)) {
            $item_id = create_guid();
            $content_id = create_guid();
            $db->query("INSERT INTO kdeploypack_items (id,systemdeploymentpackage_id,ckey,ctype,date_modified) VALUES ('$item_id','$this->id','$source','file', sysdate())");
    ...
    function getList($data){
        ...
        $total = $db->fetchByAssoc($db->query("SELECT COUNT(id) cnt FROM systemdeploymentpackages WHERE deleted = 0"));
        $res = $db->limitQuery("SELECT * FROM systemdeploymentpackages WHERE deleted = 0 ORDER BY date_entered DESC",$start,50);
        $auth_rights = 0;
    ...
    function saveRP($data){
        ...
        $this->software_hash = md5($data['software_name']. SpiceConfig::getInstance()->config['site_url']);
        ...
        $oldCRs = array();
        $res = $db->query("SELECT kdeploymentcr_id FROM krp_kcr WHERE systemdeploymentpackage_id = '" . $data['id'] . "' AND deleted = 0");
        ...
        foreach ($crs as $cr_id) {
            ...
            if(empty($oldCRs[$cr_id])){
                $oldCRs[$cr_id] = null;
                unset($oldCRs[$cr_id]);
                $db->query("INSERT INTO krp_kcr (id, systemdeploymentpackage_id, kdeploymentcr_id, date_modified, deleted) VALUES (UUID(), '" . $data['id'] . "', '$cr_id', '$modified', 0)");
            }else{
                $oldCRs[$cr_id] = null;
                unset($oldCRs[$cr_id]);
            }
        ...
        foreach($oldCRs as $key => $cr_id){
            $db->query("UPDATE krp_kcr SET deleted = 1 WHERE kdeploymentcr_id = '$cr_id' AND systemdeploymentpackage_id = '" . $data['id'] . "' AND deleted = 0");
        }
    ...
    function getCRs($data){
        $db = DBManagerFactory::getInstance();
        $list = array();
        $res = $db->query("SELECT cr.* FROM kdeploymentcrs cr INNER JOIN krp_kcr rp ON rp.kdeploymentcr_id = cr.id AND rp.systemdeploymentpackage_id = '".$data['id']."' WHERE cr.deleted = 0 AND cr.crstatus = 3 ORDER BY cr.date_entered DESC");
        while ($row = $db->fetchByAssoc($res)) {
            $list[] = array(
                'name' => $row['name'],
                'id' => $row['id'],
