...
function portal_validate_authenticated($session_id)
{
    ...
    if (session_start()) {
        ...
        if (!empty($_SESSION['is_valid_session']) && $_SESSION['ip_address'] == query_client_ip() && $valid_session != null && ($_SESSION['type'] == 'contact' || $_SESSION['type'] == 'lead' || $_SESSION['type'] == 'portal')) {
            $current_user = AuthenticationController::getInstance();
            $current_user->setCurrentUser(BeanFactory::getBean('Users'));
            $current_user->getCurrentUser()->retrieve($_SESSION['portal_id']);
            login_success();
...
function portal_get_note_attachment($session, $id)
{
    ...
    $current_user = $seed_user;//todo-uebelmar unclear situation with current_user ??
